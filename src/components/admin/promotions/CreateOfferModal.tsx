...
const steps = ['Type', 'Details', 'Rules', 'Schedule', 'Review'];
const defaultFormState = { title: '', type: 'Percentage', value: 10, status: 'Scheduled', startDate: '', endDate: '' };

const CreateOfferModal: React.FC<CreateOfferModalProps> = ({ isOpen, onClose, onSave, promotionToEdit }) => {
    const [currentStep, setCurrentStep] = useState(0);
    const [formData, setFormData] = useState<Partial<Omit<Promotion, 'id' | 'usageCount'>>>(defaultFormState);
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 2:
                return <div><h3 className="font-semibold mb-4">Eligibility Rules (Placeholder)</h3><p className="text-gray-500">Define which customers, products, or order values are eligible for this offer.</p></div>;
            case 3:
                 return (
                    <div>
