...
const EditProductPanel: React.FC<EditProductPanelProps> = ({ product, isAdding, onClose, onSave }) => {
    const isOpen = !!product || isAdding;
    const title = isAdding ? 'Add New Product' : 'Edit Product';
                        ...
                        <div className="flex-1 p-6 overflow-y-auto space-y-6">
                            ...
                            <div className="p-4 border rounded-lg">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" name="name" value={formData.name || ''} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Description</label>
                                        <textarea name="description" value={formData.description || ''} onChange={handleInputChange} rows={3} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="text-sm font-medium">Category</label>
                                            <select name="category" value={formData.category || 'Medicines'} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md">
                                         ...
                                         <div>
                                            ...
                                            <input type="text" name="tags" value={formData.tags?.join(', ') || ''} onChange={(e) => setFormData(prev => ({...prev, tags: e.target.value.split(',').map(t => t.trim())}))} className="w-full mt-1 p-2 border rounded-md" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                             ...
                             <div className="p-4 border rounded-lg">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="price" value={formData.price || 0} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Original Price (Optional)</label>
                                        <input type="number" name="originalPrice" value={formData.originalPrice || 0} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Stock</label>
                                        <input type="number" name="stock" value={formData.stock || 0} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="p-4 border rounded-lg">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <ToggleSwitch checked={!!formData.prescriptionRequired} onChange={(checked) => handleToggleChange('prescriptionRequired', checked)} />
                                </div>
                            </div>
                            ...
                            <div className="p-4 border rounded-lg">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <textarea value={formData.keyFeatures?.join('\n') || ''} onChange={(e) => handleArrayChange('keyFeatures', e.target.value)} rows={4} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Benefits (one per line)</label>
                                        <textarea value={formData.benefits?.join('\n') || ''} onChange={(e) => handleArrayChange('benefits', e.target.value)} rows={4} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Directions for Use</label>
                                        <textarea name="directionsForUse" value={formData.directionsForUse || ''} onChange={handleInputChange} rows={3} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                    <div>
                                        <label className="text-sm font-medium">Dosage</label>
                                        <input type="text" name="dosage" value={formData.dosage || ''} onChange={handleInputChange} className="w-full mt-1 p-2 border rounded-md" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-3">
                                    {activeIngredients.map((ing, index) => (
                                        <div key={index} className="flex items-center gap-2">
                                            <input type="text" placeholder="Ingredient Name" value={ing.name} onChange={(e) => handleIngredientChange(index, 'name', e.target.value)} className="w-1/2 p-2 border rounded-md"/>
                                            <input type="text" placeholder="Benefit" value={ing.benefit} onChange={(e) => handleIngredientChange(index, 'benefit', e.target.value)} className="w-1/2 p-2 border rounded-md"/>
